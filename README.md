...
Эти переменные — **без префикса `VITE_`**, специально: Vite встраивает `VITE_*` переменные в клиентский бандл, а токен бота должен остаться только на сервере (доступен исключительно внутри `api/*.js`).

Без этих переменных форма покажет ошибку отправки (и подскажет написать в Telegram напрямую) — сайт при этом не падает.

## Живой intake-бот в Telegram

Помимо формы на сайте, бот умеет вести диалог сам — если клиент пишет ему напрямую в Telegram (а не через сайт), он по шагам спрашивает тип проекта → бюджет → сроки → контакты (`api/telegram-webhook.js`) и в конце присылает тебе готовую заявку в тот же чат, что и форма. Состояние диалога не хранится ни в какой базе — каждый следующий вопрос содержит уже собранный чек-лист ответов, и бот восстанавливает его из текста сообщения, на которое клиент отвечает (Telegram reply).

Чтобы включить: после деплоя один раз зарегистрируй вебхук (замени токен и секрет на свои):

```bash
curl -X POST "https://api.telegram.org/bot<ТВОЙ_ТОКЕН>/setWebhook" \
  -H "Content-Type: application/json" \
  -d '{"url":"https://<твой-домен>/api/telegram-webhook","secret_token":"<TELEGRAM_WEBHOOK_SECRET>"}'
```

`TELEGRAM_WEBHOOK_SECRET` — просто случайная строка для проверки, что запрос на `/api/telegram-webhook` действительно пришёл от Telegram, а не от постороннего.

## Деплой на Vercel

Preset: **Vite**, Root Directory: `/`. Vercel сам подхватит файлы из `api/` как serverless-функции — ничего дополнительно настраивать не нужно. Добавь те же переменные окружения в настройках проекта Vercel (Production + Preview + Development).
